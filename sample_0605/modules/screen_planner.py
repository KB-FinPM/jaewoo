...
반드시 JSON 배열만 반환한다.
각 항목 schema:
[{"requirement_id":"REQ-0001","screen_id":"SCR-001","screen_no":"SCR-001","screen_name":"화면명","screen_summary":"화면 기획 요약","display_items":[{"item_name":"표시항목명","description":"화면에 표시해야 할 내용 설명"}]}]
규칙:
...
def _compact_requirements(atoms: List[RequirementAtom]):
    return [{'requirement_id': a.requirement_id, 'category': a.category, 'biz_requirement_name': a.biz_requirement_name, 'requirement_name': a.requirement_name, 'requirement_type': a.requirement_type, 'domain': a.domain, 'feature': a.feature, 'description': a.description} for a in atoms]


def generate_screen_items_for_domain(domain: str, atoms: List[RequirementAtom], instruction_md: str = '') -> List[ScreenPlanItem]:
    agent_instruction = load_agent_instruction(instruction_md)
    system_prompt = f"{SCREEN_SYSTEM_PROMPT}\n\n{agent_instruction}".strip()
...
def generate_screen_plan_items_from_rag(domain_contexts: Dict[str, List[RequirementAtom]], instruction_md: str = '') -> List[ScreenPlanItem]:
    all_items = []
    for idx, (domain, atoms) in enumerate(domain_contexts.items(), start=1):
        ...
        all_items.extend(generate_screen_items_for_domain(domain, atoms, instruction_md=instruction_md))
