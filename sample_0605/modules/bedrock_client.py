...
from modules.config import AWS_ACCESS_KEY_ID, AWS_CA_BUNDLE, AWS_REGION, AWS_SECRET_ACCESS_KEY, AWS_VERIFY_SSL, BEDROCK_MODEL_ID, BEDROCK_READ_TIMEOUT
...
session_kwargs = {
    ...
    'verify': AWS_CA_BUNDLE if AWS_CA_BUNDLE else AWS_VERIFY_SSL,
...
def invoke_bedrock(system_prompt: str, user_prompt: str, max_tokens: int = 4000, retry_count: int = 3) -> str:
    ...
    for attempt in range(1, retry_count + 1):
        try:
            response = bedrock_client.invoke_model(modelId=BEDROCK_MODEL_ID, body=json.dumps(request_body))
