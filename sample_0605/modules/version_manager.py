...
from modules.s3_client import ensure_local_path, list_s3_keys
...
def find_latest_versioned_docx(
    ...
    if directory.exists():
        for file_path in directory.glob('*.docx'):
            ...
            if not match:
                continue
            candidates.append((int(match.group(1)), str(file_path)))

    bucket_name = os.getenv('S3_BUCKET_NAME')
    if not candidates and bucket_name and os.getenv('S3_STORAGE_BACKEND', 's3') == 's3':
        prefix = os.getenv('S3_UPLOAD_PREFIX', 'storage/upload_files')
        for key in list_s3_keys(prefix, bucket_name=bucket_name):
            file_name = Path(key).name
            match = pattern.match(unicodedata.normalize('NFC', file_name))
            if not match:
                continue
            candidates.append((int(match.group(1)), f's3://{bucket_name}/{key}'))
    ...
    return ensure_local_path(selected)
