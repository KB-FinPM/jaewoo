...
def _replace_placeholders(ws, values: Dict[str, str], max_token: int = 0):
    for row in ws.iter_rows():
        for cell in row:
            ...
            cell.value = limit_text_for_output(text, max_token=max_token)
...
def _apply_placeholder_sheets(wb, placeholder_sheets: List[Dict[str, Any]], context: Dict[str, str], max_token: int = 0):
    for sheet_mapper in placeholder_sheets:
        ...
        _replace_placeholders(wb[sheet_name], values, max_token=max_token)


def _write_data_sheet(wb, items: List[Any], data_mapper: Dict[str, Any], context: Dict[str, str], max_token: int = 0):
    ...
    for row_offset, item in enumerate(items):
        ...
        for col_no, column_mapper in resolved_columns:
            ...
            ws.cell(row=excel_row, column=col_no).value = limit_text_for_output(value, max_token=max_token)
...
def save_requirement_excel(
    ...
    _apply_placeholder_sheets(wb, mapper.get('placeholder_sheets', []), context, max_token=max_token)
    _write_data_sheet(wb, atoms, mapper['data_sheet'], context, max_token=max_token)
...
def save_wbs_excel(
    ...
    wb = load_workbook(resolve_path(mapper.get('template_path') or template_path))
    _write_data_sheet(wb, items, mapper['data_sheet'], context={}, max_token=max_token)
