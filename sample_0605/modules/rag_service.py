...
def unique_domains(atoms: List[RequirementAtom]) -> List[str]:
    ...
    seen = set()
    for atom in atoms:
        domain = (atom.biz_requirement_name or atom.domain or '공통').strip() or '공통'
...
def fallback_domain_atoms(atoms: List[RequirementAtom], domain: str, limit: int = 25) -> List[RequirementAtom]:
    matched = [a for a in atoms if (a.biz_requirement_name or a.domain or '공통').strip() == domain]
    return matched[:limit] if matched else atoms[:limit]


def retrieve_atoms_for_domain(store: Any, all_atoms: List[RequirementAtom], doc_key: str, domain: str, purpose: str, limit: int = 25) -> List[RequirementAtom]:
...
def build_domain_contexts(store: Any, all_atoms: List[RequirementAtom], doc_key: str, purpose: str, limit_per_domain: int = 25) -> Dict[str, List[RequirementAtom]]:
    contexts = {}
