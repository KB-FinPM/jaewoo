...
`process.json`의 `max_token` 값은 최종 output 산출물에 기록되는 텍스트 길이에만 적용한다. JSON Bedrock 응답/복구 원문은 파싱 오류 방지를 위해 제한하지 않는다.
기본값은 `1000`이며, 문장/공백 경계를 우선하여 자르기 때문에 100~200 토큰 정도는 문맥 유지를 위해 유연하게 처리한다.
